...
class Test_DuplicateTagError(TestCase):
    def test_str_output(self):
        expected = 'DuplicateTagError: the tag "badtag" already exists in the repository'
...
class Test_Basic_Rpository_Operations_On_MemoryRepository(TestCase):
    ...
    def test_tags_are_unique(self):
        ...
        def will_raise_DuplicateTagError():
            self.repo.create_commit_from_filename('some tag', self.file1) 
    ...
    def test_message_character_set_limited_to_alphanumeric_and_underscore(self):
        def will_raise_Exception():
            self.commit_file1('a', message,'some illegal data')
