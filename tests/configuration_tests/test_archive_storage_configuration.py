...
class Test_Storage_Configurations(TestCase):
    ...
    def test_default_Archive_storage_is_S3(self):
        self.assertEqual('s3', config().get('Snapshot', 'driver'))
...
class Test_StorageFactory_From_Configuration(TestCase):
    ...
    def configure_driver(self, driver_type):
        self.config.set('Snapshot', 'driver', driver_type)
...
class Test_MemoryCommit_Storage(TestCase):
    def setUp(self):
        self.config = config()
        self.config.set('Snapshot', 'driver', 'memory')
...
class Test_FilesystemCommitStorage(TestCase):
    def setUp(self):
        ...
        self.config = config()
        self.config.set('Snapshot', 'driver', 'filesystem')
        self.config.set('Snapshot', 'path', self.commit_storage_path)
...
class Test_S3CommitStorage(TestCase):
    def setUp(self):
        self.config = config()
        self.config.set('Snapshot', 'driver', 's3')
