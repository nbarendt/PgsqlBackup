...
class Test_Storage_Configurations(TestCase):
    ...
    def test_default_Archive_storage_is_S3(self):
        self.assertEqual('s3', config().get('Archive storage', 'driver'))
...
class Test_StorageFactory_From_Configuration(TestCase):
    ...
    def configure_driver(self, driver_type):
        self.config.set('Archive storage', 'driver', driver_type)
    ...
    def test_memory_driver_factory_called(self, factory_mock):
        ...
        get_repository_storage_from_config(self.config, 'Archive storage')
        factory_mock.assert_called_with(self.config, 'Archive storage')
...
class Test_MemoryCommit_Storage(TestCase):
    def setUp(self):
        self.config = config()
        self.config.set('Archive storage', 'driver', 'memory')

    def test_will_build_storage_from_config(self):
        self.assertEqual(MemoryCommitStorage,
            type(get_repository_storage_from_config(self.config, 'Archive storage')))
...
class Test_FilesystemCommitStorage(TestCase):
    def setUp(self):
        ...
        self.config = config()
        self.config.set('Archive storage', 'driver', 'filesystem')
        self.config.set('Archive storage', 'path', self.commit_storage_path)
...
class Test_S3CommitStorage(TestCase):
    def setUp(self):
        self.config = config()
        self.config.set('Archive storage', 'driver', 's3')
    ...
    def test_will_raise_MissingS3Configuration_without_prefix(self):
        self.config.set('Archive storage', 'bucket', 'somebucket')
    ...
    def test_will_build_storage_from_config(self, mock_get_s3_connection):
        self.config.set('Archive storage', 'bucket', 'somebucket')
        self.config.set('Archive storage', 'prefix', 'someprefix')
